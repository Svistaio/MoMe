...
\begin{document}
    ...
    \title{
        \vspace*{-2cm}
        \huge\textbf{
            Analysis of brain network models for Alzheimer's disease via spreading of prion-like tau misfolded proteins
        }
    }
        ...
        \begin{@twocolumnfalse}
            ...
            \begin{center}
                \noindent\parbox{0.9\textwidth}{%
                    \begin{abstract}
                        \noindent The prion-like hypothesis is assumed to model Alzheimer's disease via the Fisher-Kolmogorov, hetorodymer and Smoluchowski models. These are initially defined in a continuous setting, and later discretized both in space, which requires a study on timescales, and time; afterwards, an asymptotic analysis is discussed along side the equilibrium points and an empirically proposed function to estimate the convergence time. Finally, they are complicated by considering time-varying diffusion for all models, accumulation and size dependency only for the Smoluchowski one; the results discussed are in line in line with the theory developed.
                    \end{abstract}
                }
            \end{center}
...
%%%
    ...
    \section{Introduction} \label{secIntroduction}
        ...
        These mechanisms are based on prion diseases firstly observed in Bovine Spongiform Encephalopathy (BSE), more commonly known as mad cow disease, and can be applied more broadly to other neurodegenerative disease (for instance Parkinson's).
        
        Bear in mind this article focuses only on the dynamic of misfolded tau proteins and not on its damage to the brain, which would require another set of equations to predict brain atrophy using the toxic protein concentrations (see \cite{Bertsch2016} as an example).
...
%%%
    ...
    \section{Kinetic models} \label{secKineticModels}
        ...
        To predict the concentration of misfolded $\tau$ proteins, Three continuous models are considered, followed by spatial and temporal discretization methods. Afterwards, an asymptotic analysis is discussed to normalize the results.

        We assume the brain is contained in the closed region $\mathcal B\subset\mathbb R^3$ and that it's observed in a time interval $\mathcal T\equiv(0,T]$ of duration $T\in\RPlusStar$; it's also handy to define
        %
        \begin{equation}
            \begin{aligned}
        ...
        \end{equation}
        %
        as the space-time cylinder, its space/time boundaries and $\partial\mathcal T_-\equiv\{0\}$. Lastly, we set
        %
        ...
        \end{equation}
        %
        as the positive real numbers with and without the origin.
        

        \subsection{Continuous models}\label{secContinuousModels}
        
            These models exhibit different levels of complexity depending on the subset of assumptions \ref{hypPrionHypothesisMisfolded}-\ref{hypPrionHypothesisAggregate} implemented: all three considers \ref{hypPrionHypothesisMisfolded} and \ref{hypPrionHypothesisTemplate} but only the last one takes into account \ref{hypPrionHypothesisAggregate}. \cite{Fornari2019,Goriely2019}

            A shared and fundamental feature is the nonlinear nature due to the quadratic reaction term necessary to model \ref{hypPrionHypothesisTemplate}.
            ...
            \subsubsection{The heterodimer model (HD)}\label{secContinuousModelsHD} 
                ...
                \end{equation}
                %
                where $\DeltaD f\equiv\nabla\cdot(\mathbf D\cdot\nabla f)$ is the tensorial Laplacian operator\footnotemark{}, with the diffusion tensor $\mathbf D$, $\gamma$ is the production rate of healthy protein, $\mu_h$ and $\mu_m$ are the clearance rates of healthy and misfolded proteins respectively, and $\kappa$ is the misfolding conversion rate. 

                \footnotetext{$\DeltaD$ is equal to the classical Laplacian operator iff $\mathbf D$ is isotropic: $\DeltaD f=D\Delta f$, \ie{} $\mathbf D=D\mathbf I$ with $D\in\RPlus$.}


            \subsubsection{The Fisher-Kolmogorov model (FK)}\label{secContinuousModelsFK}

                It can be derived from \HDcontinuous{} by assuming $c_h$ is homogeneous and stationary, namely that it's already in an equilibrium point (see Appendix \ref{apxFKFromHD} for more details).
                ...
                \end{equation}
                %
                where $c$ is the dimensionless concentration of misfolded proteins and $\alpha$ represents the misfolding conversion rate.
            ...
            \subsubsection{The Smoluchowski model (SMO)}\label{secContinuousModelsSMO}
                ...
                \end{equation*}
                %
                where $a_{ij}$ and $f_{ij}$ are respectively the aggregation and fragmentation rates. Hence, the collective effect of aggregation and fragmentation can be summarized by
                %
                \begin{equation}
                    \begin{aligned}
                        A_i&=A^g_i-A^l_i=
                ...
                \end{equation}
                %
                where $k_{0i}$ and $\mu_i$ have the usual meaning of production and clearance rate already seen in \HDcontinuous{}.
                ...
                \begin{enumerate}[
                    ...
                    \item\label{hypSMOModelProduction} production is only possible with healthy mo\-no\-mers: $$\gamma_i=0\;\;\forall i\in\NPlus\setminus\{1\}$$ while $\gamma\equiv\gamma_1$;
                    \item\label{hypSMOModelClearance} clearance is size-independent with rate $$\mu_i\equiv\mu\text{, with }1\le i\le L;$$
                    ...
                    \item\label{hypSMOModelAggregation} aggregation is size-independent, that is $$a_{ij}=a\text{ with }1\le i,j<L,$$ but can only occur by adding single monomers: $$a_{ij}=0\text{ with }1<i,j<L;$$
                    \item\label{hypSMOModelFragmentation} fragmentation into monomers is impossible, \ie{} $$f_{ij}=0\text{ with }i=1\text{ or }j=1,$$ whereas fragmentation into smaller polymers is size-in\-de\-pen\-dent: $$f_{ij}=f\text{ with }1<i,j<L.$$
                \end{enumerate}
                ...
                \end{equation}
                %
                where $\boldsymbol\kappa\in L_+^\infty(\Brain)$, with
                %
                \begin{equation*}
        ...
        \subsection{Discrete models}\label{secDiscreteModels}
            ...
            For the spatial discretization graph theory is applied to take full advantage of \ref{hypPrionHypothesisAxons}; in addition, characteristic times have to be considered for a biologically sound dynamic. Instead, for the temporal discretization Euler methods are analysed.
            ...
            \subsubsection{Spatial discretization}\label{secSpatialDiscretization}
                ...
                \begin{itemize}[%
                    ...
                    \item the degree matrix $$\mathbf D=\diag(\mathbf A\bOne),$$ where $\bOne$ is the unitary vector in $\mathbb R^V$: it's a diagonal matrix whose en\-tries $$d_{ii}\in\RPlus\text{, with }1\le i\le V,$$ are the sum of the rows of $\mathbf A$.
                \end{itemize}
            
                The difference of the latter with the former is defined as the Laplacian matrix:
                %
                \begin{equation}
                    \mathbf L=\rho(\mathbf D-\mathbf A)=\rho\boldsymbol{\mathcal L},
                    \label{eqLaplacianMatrix}
                \end{equation}
                %
                where $\rho$ is a constant to balance the diffusion and growth phenomena, as is going to be explained shortly in § \ref{secCharacteristicTimes}. The Laplacian matrix is indeed the discretization of the ten\-so\-rial Laplacian operator: $-\mathbf L\longleftrightarrow\DeltaD$; as an example, remember that $$\DeltaD c=0\ \iff\ c(\mathbf x,t)=c\;\;\forall\mathbf x\in\mathbb R^3,\ t\text{ fixed},$$ and this behaviour is indeed reproduced with $\mathbf L$ if applied to a uniform vector $\mathbf c=c\bOne$:
                %
                \begin{equation*}
                    \mathbf L\mathbf c=
                    c\mathbf L\bOne=
                ...
                To build the Laplacian one can start from a more fine brain network representation; nonetheless, for the sake of simplicity, in this article the Laplacian used in \cite{Goriely2019,Goriely2019_L}, shown in Fig. \ref{figMatricesLDA}, is considered. %(see Appendix \ref{apxBudapestConnectome})
                
                \begin{figure*}[tb]
                    \centering
                    \resizebox{\textwidth}{!}{\input{../Figure/Matrici.tex}}
                ...
                \end{equation}
                %
                where $\HadProd$ is the element-wise product (Hadamard product); for \HDcontinuous{}
                %
                \begin{equation}
                    \text{[HD]$_d$}\left\{
                    \begin{alignedat}{2}
                ...
                In all previous models $\mathbf q\in\{0,1\}^V$ is a vector with $q$ unitary entries describing which nodes have the initial toxic mass or where nucleation takes place.

                Observe how the number of unknowns is just the number of ROI values multiplied by the number of vertices: $V$, $2V$ and $LV$ respectively; indeed, thanks to the discretization in space, each function went from having an infinite amount of degrees of freedom to at most $LV$.
            ...
            \subsubsection{Characteristic times}\label{secCharacteristicTimes}
                ...
                In all models previously seen, for tau misfolded proteins there are two important phenomena at play:
                ...
                \begin{enumerate}[
                    ...
                    \item\label{hypGrowthTimeHypotesisLinear} the nonlinear terms are neglected, namely low concentrations\footnotemark{} are studied to simplify calculations by linearization;
                ...
                These hypotheses give the following growth characteristic times:
                ...
                \begin{description}[style=unboxed,
                    ...
                    \end{equation}
                    %
                    where $\lambda^{\text{min}}_{\mathbf A}$ is the minimal absolute eigenvalue of $\mathbf A$.

                \end{description}

                With these definitions, three cases are described based on the relative value between $\tau_d$ and $\tau_d$:
                ...
                \includetikzgroupplotfigure*{t}{0pt}{\textwidth}
                    ...
                    {Three timescale cases in \FKdiscrete{} with $\alpha=0.5$
                    (see Tab. \ref{tabTimeScales} for the values).}
                    {figTimeScales}
                ...
                \begin{table}[htb]
                    ...
                    S[
                        ...
                        $\tau_d\ll\tau_g$    &1e-10&{$\approx$}2e8 &2e0&92.35\\[\hRow]
                        $\tau_d\approx\tau_g$&1e-3 &{$\approx$}2e1 &2e0&29.69\\[\hRow]
                        $\tau_d\gg\tau_g$    &1e0  &{$\approx$}2e-2&2e0&20.04\\[\hRow]
                    \end{NiceTabular}
            ...
            \subsubsection{Temporal discretization}\label{secTemporalDiscretization}
                ...
                \end{equation}
                %
                for a fixed Laplacian $\mathbf L$, set of parameters in $\mathcal P$ and where, due to the quadratic terms, $\mathbf F$ is nonlinear.

                Given a uniform time step size $\Delta t\in\RPlusStar$ such that $$T/\Delta t=N\in\NPlus,$$ then $\mathcal T$ is discretized with $t_n=n\Delta t$.
                ...
                Conversely, explicit methods are less computationally demanding with respect to implicit ones as the latter have to solve a root-finding problem for the next value since $F(\mathbf c(t),t;\mathbf L,\mathcal P)$ is nonlinear. \cite{MACNMonegato2008,NMFEDormand2018}
                ...
                \end{equation}
                %
                $\forall n\in\{1,\ldots,N-1\}$, which now lies in $\tilde c\in L_+^\infty(\STCyl)$.

        \subsection{Asymptotic analysis}\label{secAsymptoticAnalysis}
            ...
            \subsubsection{FK model}\label{secAsymptoticAnalysisFK}
                ...
                \end{equation}
                %
                whose EPs are $c_\infty=0,1$, as $c$ is, in fact, already dimensionless; however from the derivative
                %
                \begin{equation*}
                ...
                \end{equation}
                %
                where each equation is like \eqref{eqFKContinuousAsymptoticEquation} but with $c^i_\infty$, $1\le i\le V$, in place of $c_\infty$; moreover, since the system is homogeneous, it holds $\mathbf c_0=c_0\mathbf q=c_0\mathbf I$ with $c_0\neq0$, so every node tends towards $1$ and the asymptotic toxic vector is $\mathbf c_\infty=\mathbf I$.
            ...
            \subsubsection{HD model}\label{secAsymptoticAnalysisHD}
                ...
                \end{equation}
                %
                where for each node is described by a system like \eqref{eqHDContinuousAsymptoticSystem} but with $c^{i,\infty}_h$ and $c^{i,\infty}_m$, $1\le i\le V$, in place of $c^\infty_h$ and $c^\infty_m$. In addition, since the system is homogeneous, it holds $\mathbf c_{m,0}=c_{m,0}\mathbf q=c_{m,0}\mathbf I$ with $c_{m,0}\neq0$, so every vertex tends towards $\mathbf c_2$ and the asymptotic toxic vector is $\mathbf c_m^\infty=c_{m,2}^\infty\mathbf I$.
            ...
            \subsubsection{SMO model}\label{secAsymptoticAnalysisSMO}
                ...
                To gather the necessary equations the following assumption are made: the number of tau polymers $L$ is sufficiently large as to not affect the dynamics on intermediate time scales of disease progression. \cite{Goriely2019} In essence it means that the sum over all equations in \SMOcontinuous{} is well approximated by the sum in the $L\to\infty$ limit.

                Consequently, a new system of three equations can be derived by considering the first equation in \SMOcontinuous{}, with $m\equiv c_1$, the sum over all equations with $i>2$ in \SMOcontinuous{} and likewise the third but with each equation multiplied by $i$ (see Appendix \ref{apxSystemOfMoments}):
                %
                \begin{subequations}
                ...
                \end{equation}
                %
                The roots of \eqref{eqmInfEstimateEquation} can be found with Cardano's formula \cite{AlgebraVanWaerden2003,HAKurosh1972}, although, due to the solution complexity, their symbolic form is of little use; indeed, one then needs to use them to find $\MInf$ and $\PInf$, with \cref{eqMInfValue,eqPInfValue} respectively, and finally to evaluate the Jacobian of \eqref{eqAsymptoticMomentEquations} to establish their nature as LAS or unstable. Therefore, the analysis of the EP nature is omitted.
                % https://encyclopediaofmath.org/wiki/Cardano_formula

                Nevertheless, in the cases studied using the parameters given by \cite{Fornari2019,Goriely2019} we can at least numerically say the solution satisfying the positivity constraint (see Appendix \ref{apxSMOPositivityAsmptoticConstraint}) is the smallest root of \eqref{eqmInfEstimateEquation}; this result seems to be confirmed [but not proven\footnotemark{}] by \cite{Goriely2019}.

                \footnotetext{Indeed, in § 5.5 on p. 10 they only give upper and lower bounds for $\mInf$ without studying the EP nature.}
                ...
                \end{equation}
                %
                Thus the formulas for the discrete asymptotic values are the same as \cref{eqMInfValue,eqPInfValue,eqmInfEstimateEquation}, but with a different $\varphi$ definition and valid at each vertex so that the network values are $V$ times the previous ones.

                To conclude it's also possible to estimate the asymptotic size distribution \cite{Goriely2019} (see Appendix \ref{apxAsymptoticSizeDistribution}) with
                %
                \begin{equation}
                    c^\infty(n)=\PInf K(n)
                ...
                \end{equation*}
                %
                and the maximum concentration is reached by the polymer with size closest to
                %
                \begin{equation}
            ...
            \subsubsection{Convergence time}\label{secAsymptoticAnalysisNetwork}
                ...
                One might find reasonable to find the time necessary for the three models to reach their respective asymptotic values, based on their parameters $\mathcal P$.
                ...
                We here propose one such function based empirically on the cases in \cite{Fornari2019,Goriely2019}:
                %
                \begin{equation}
                ...
                \begin{figure}[tb]
                    \centering
                    \resizebox{\linewidth}{!}{\input{../Figure/FunzioneContributoDiffusivo.tex}}
                    \captionsetup{width=0.5\linewidth}
...
%%%
    ...
    \section{Results and comparisons}\label{secResultsComparisons}

        In this section we first introduce complications to the base models seen in § \ref{secKineticModels}, then parameters are set and finally the results are discussed comparing the base models with and without complications.
        ...
        \subsection{Complications}\label{secComplications}
            ...
            \subsubsection{Dynamic Laplacian}\label{secComplicationsDynamicLaplacian}
                ...
                Ageing is a natural and inevitable process in all our lives, and one crucial way it manifest is in the weakening of the body. Thus, the fact that the Laplacian in \eqref{eqLaplacianMatrix} does not depend on time is in fact a mere simplification.
                ...
                \begin{enumerate}[
                    ...
                    \item\label{bhvSpecificDecay} or edges depends on some quantity: the bigger it is, the faster/slower they decay in time.
                \end{enumerate}
                ...
                As the control quantity one might propose the distance, but due to the high anisotropy of the brain this is a poor choice. A better choice are the entries of the adjacency matrix $\mathbf A$ (Fig. \ref{figMatricesLDA}) themselves, which are defined as the ratio of the number of fibres over the average fibre length: $n/l$ \cite{BRCI,BRCII,BRCIII}; therefore the larger is $A_{ij}$ the greater the brain will try to maintain it.

                \begin{figure}[tb]
                    \centering
                    \resizebox{\linewidth}{!}{\input{../Figure/FunzioniDecadimento.tex}}                    
                    \captionsetup{width=0.5\linewidth}
                    \caption{Decay functions.}
                    \label{figDecayFunctions}
                \end{figure}

                To classify the entries as low/medium/high value the maximum $A_{\max}=0.2147$, minimum $A_{\min}=0$ and average
                %
                \begin{equation*}
                    \overline A\equiv\frac1{V^2}\sum_{i,j}^VA_{ij}
                    =\num[scientific-notation=true]{0.0029}
                \end{equation*}
                %
                values of $\mathbf A$ are considered; then the midpoints between $\overline A$ and $A_{\max}/A_{\min}$ defines the thresholds needed to classify the entries:
                %
                \begin{equation}
                    \begin{split}
                        S_{lm}&\equiv\frac{A_{\min}+\overline A}2
                        =\num[scientific-notation=true]{0.0015},\\
                ...
                In total the number of low/medium/high value edges are \num{5785}, \num{1094} and \num{10} respectively, which sum to $89^2$ as expected. With all this, $\mathbf A(t)$ is defined as follows
                %
                \begin{equation}
                    \newcommand\vspcR{2pt}
                    A_{ij}(t)\equiv
                ...
                %
                \begin{equation}
                    \mathbf L(t)=\rho(\mathbf D(t)-\mathbf A(t))=\rho\boldsymbol{\mathcal L}(t),
                    \label{eqDynamicLaplacianMatrix}
                \end{equation}
                %
                but with $\mathbf D(t)\equiv\diag(\mathbf A(t)\boldsymbol1)$.
                
                It's noteworthy to say that the kinematics described by \eqref{eqTimeDependentA} is purely theoretical and based on the assumption that the low value edges decay very rapidly (exponential decay), whereas those medium and high value are more resistant (hyperbolic decay).
            ...
            \subsubsection{Size-dependency}\label{secComplicationsSizeDependency}
                ...
                For diffusion, assumption \ref{hypSMOModelDiffusion} can be changed so that the diffusion coefficient $\mathbf D_i$ of the $i$-th polymer scales inversely to its molecular weight, which is proportional to its size: \cite{Goriely2019}
                %
                \begin{equation}
                ...
                For clearance, assumption \ref{hypSMOModelClearance} can be modified by setting $\mu_i=\mu/i$; in this way, the larger the polymer the slower it will diffuse. This last change actually breaks mass conservation in \eqref{eqMassConservation} which becomes \cite{Goriely2019}
                %
                \begin{equation}
                ...
                \end{equation}
                %
                where $\TotP\equiv\sum_{i=1}^Lc_i=c_1+P$, so mass is expected to grow even if $\TotM(0)=\gamma/\mu$. The same result [but not \eqref{eqPTotEquation}] is true, to a lesser extent, with the previous complication in which only $\mu_L=0$ is different.
        ...
        \subsection{Settings}\label{secSettings}
            ...
            In this section the parameters for \FKdiscrete{}, \HDdiscrete{} and \SMOdiscrete{} are given and, in few words, they all come from \cite{Fornari2019}; further testing of the code for the SMO model has also been done with those taken from \cite{Goriely2019}. However, two settings are shared between all models:
            ...
            \begin{itemize}[%
                ...
                \item the infected nodes, \ie{} where the initial toxic conditions are set (§ \ref{secDiscreteModels}), in the entorhinal cortex ($q=2$) in the Limbic system, shown in Fig. \ref{figInfectedNodes};
            ...
            In conclusion to this section, some remarks are necessary regarding all the parameters taken from \cite{Fornari2019}.
            ...
            \subsubsection{FK model}\label{secSettingsFK}
                ...
                \begin{itemize}[%
                    ...
                    \item the Euler implicit method in \eqref{eqUpdateRuleEI} is considered with a time step size of $\Delta t=\qty{0.4}{\year}$.
                \end{itemize}
            ...
            \subsubsection{SMO model}\label{secSettingsSMO}
                ...
                \begin{itemize}[%
                    ...
                    %
                    %
                    \item the initial healthy concentration is $c_{1,0}=\qty2{\molarity}=\gamma/\mu$, while the nucleation rate is non-null takes value
                    %
                    \begin{equation}
                        \kappa=\qty{1.6e-4}{\per\molarity\per\year};
                        % \kappa=1.6\times10^{-4}\ 
                        % \Molarity\PowerMinOne\cdot\Year\PowerMinOne.
                        \label{eqSMONucleationRate}
                    \end{equation}
                    %
                    \item the Euler explicit method in \eqref{eqUpdateRuleEE} is considered with a time step size of $\Delta t=\qty{0.04}{\year}$ to ensure its stability.
                \end{itemize}
        ...
        \subsection{Results}\label{secResults}
            ...
            % \begin{figure*}[t]
            %     \centering
            %     \resizebox{\textwidth}{!}{\input{../Figure/FKEDCurve.tex}}
            %     % \captionsetup{width=0.5\linewidth}
            ...
            \subsubsection{FK and HD model}\label{secResultsFKHD}
                ...
                Instead of showing the concentration of all nodes, it's better to visually pool some together and in both cases the group of ROIs considered form
                ...
                \begin{itemize}[%
                    ...
                    \item a single node (brainstem),
                    \item or the whole network.
                \end{itemize}
                ...
                \footnotetext{The authors in \cite{Fornari2019} suggests the same conclusion but do not prove it, opting instead to discuss some parametric studies of \FKdiscrete{} and \HDdiscrete{} [unfortunately without normalizing the latter].}
                ...
                The \eqref{eqLDVariantI} case can be seen in Figs. \cref{figFKDynamicLVariant1,figHDDynamicLVariant1}. From Figs. \cref{figFKNetwork,figFKTemporal,figFKFrontal,figFKParietal,figFKOccipital,figFKLimbic,figFKBasalGanglia,figFKBrainstem,figHDNetwork,figHDTemporal,figHDFrontal,figHDParietal,figHDOccipital,figHDLimbic,figHDBasalGanglia,figHDBrainstem} it's evidently a middle way between the static and dynamic $\mathbf L$ cases. Consequently, the few \num{10} high value edges are not really relevant for the diffusion.
                
                On the contrary, the \eqref{eqLDVariantII} case is shown in Figs. \cref{figFKDynamicLVariant2,figHDDynamicLVariant2} in which the basal ganglia grow before the brainstem, while the Limbic system slows down and is surpassed by the parietal lobe. From Figs. \cref{figFKNetwork,figFKTemporal,figFKFrontal,figFKParietal,figFKOccipital,figFKLimbic,figFKBasalGanglia,figFKBrainstem,figHDNetwork,figHDTemporal,figHDFrontal,figHDParietal,figHDOccipital,figHDLimbic,figHDBasalGanglia,figHDBrainstem}, instead, it's apparent this is by far the most delayed dynamic of all, save the temporal lobe.

                Therefore, the \eqref{eqLDVariantII} case hints at the importance of the \num{1094} medium value edges for the dynamic of misfolded tau proteins. One could attribute this behaviour to the bigger increase in diffusive time $\tau_d$ but Fig. \ref{figDynamicDiffusiveTimeScales} proves this may not the case: $\tau_d$ of the \eqref{eqLDVariantII} case increases more in the first half but is then surpassed in value by the \eqref{eqLDVariantI} case one; at any rate, they still remain in the same OOM of $\tau_g$, far from the hiccup evolution described in § \ref{secCharacteristicTimes}.
                
                \begin{figure}[htb]
                    \centering
                    \resizebox{\linewidth}{!}{\input{../Figure/TempoDiffusivo.tex}}
                    % \captionsetup{width=0.5\linewidth}
                    \caption{Dynamic diffusive timescale.}
                    \label{figDynamicDiffusiveTimeScales}
                \end{figure}

                Lastly it's noteworthy the fact that the convergence time increases only slightly in all cases, confirming \ref{rmkDiffusionContributionConvergenceTime}.
            ...
            \subsubsection{SMO model}\label{secResultsSMO}
                ...
                %
                \begin{equation}
                    \Mass_l\equiv\frac1{V\MInf}\sum_{v=1}^V\sum_{l=i}^Llc_l^v,\quad2\le l\le L,
                    \label{eqAggregateMass}
                \end{equation}
                %
                and the normalized size distribution, \ie{} the concentration against the size, w.r.t. $\PInf$ of the polymers. The resulting plots are shown in Fig. \ref{figSMOCurves}.

                \footnotetext{For this reason the variants discussed for the FK and HD models are omitted in this section.}

                The static $\mathbf L$ case illustrated in Fig. \ref{figSMOSDStaticL} behaves as predicted by the asymptotic analysis discussed in § \ref{secAsymptoticAnalysisSMO}; in particular, the asymptotic estimate for the size distribution is pretty close to the actual distribution and this is true even for the average polymer length defined as the weighted mean of the sizes w.r.t. their concentrations:
                %
                \begin{equation}
                    \bar l\equiv
                ...
                As expected, from the dynamic $\mathbf L$ case in Fig. \ref{figSMOAMDynamicL} it's clear the delayed nature compared to Fig. \ref{figSMOSDStaticL}; this is slight\-ly reflected in the size distribution in Fig. \ref{figSMOSDDynamicL} as well, since it's smaller than \ref{figSMOSDStaticL} and it has a greater average length than $\lambda$.
                
                Conversely, the absence of clearance of the last polymer shows its accumulation pretty well with both the presence of a toxic mass base in \ref{figSMOAMAccumulation}, while in \ref{figSMOSDAccumulation} the most noticeable difference is the spike in $n=50$ which shift notably the average length to a greater value w.r.t. $\lambda$, but overall the distribution remains the same as Fig. \ref{figSMOSDStaticL}. Observe the fact that this spike is present even in Figs. \cref{figSMOSDStaticL,figSMOSDDynamicL} too, but only temporarily, due to the absence of aggregation processes for the longest toxic polymer in \SMOdiscrete{}.
                
                Finally, the size dependency has the most drastic effects with an eight-fold increase of $\Mass_2$ in Fig. \ref{figSMOAMSizeDependent} and a five-fold one of the size distribution in Fig. \ref{figSMOSDSizeDependent}; furthermore, the major growth happens around \qty{10}{\year} before the static $\mathbf L$ case, certainly because of the lower clearance rate for all polymers. It's then to no-one's surprise the asymptotic distribution is completely wrong.  Nevertheless, it was expected from \eqref{eqPTotEquation} as the new EP is now in $\TotP=\gamma/\mu$; indeed, by plotting $\TotP$ in time, as shown in Fig. \ref{figSMOSizeDependentPTot}, one finds $\TotP<\gamma/\mu$, meaning $\dot M_{\text{tot}}>0$, for \qty{60}{\year}.

                \includetikzgroupplotfigure{H}{\linewidth}
                    {\input{../Figure/PtotDipendenzaTempo.tex}}
                    {\selezionaLingua
                        {Evoluzione di $\TotP$ nel caso dipendente dalla dimensione}
                        {$\TotP$ evolution in size-dependency case}.}
                    {figSMOSizeDependentPTot}

                This behaviour is probably due to the lower equilibrium concentration for larger polymers leading to an accumulation of the shorter ones, although a more rigorous proof is needed to verify this hypothesis, maybe akin to the moment analysis discussed in § \ref{secAsymptoticAnalysisSMO}.
...
% %%%
    ...
    All results in § \ref{secResults} are coherent with the theory explained in § \ref{secKineticModels}, although, as with most simulations, they have to be considered carefully; this is true especially from a biomedical point of view since the parameters used are purely theoretical and do not come from any study (§ \ref{secDisclaimer}).
    
    Furthermore, the Laplacian evolution in § \ref{secComplicationsDynamicLaplacian} has to compared with actual measures in vivo, which is understandably difficult to achieve. It would also be interesting to find some laws about its evolution either based on ageing or concentrations of tau proteins.
    
    Another possible improvement can be searched in dynamic parameters, but then the problem is to find their appropiate evolution; indeed, why should the nucleation rate behave like the healthy clearance rate? One could expect the former to increase whereas the latter to decrease in time in a diseased brain.
    
    All considered, this paper is better to be regarded as a formal exploration of more complex models and their phenomenology.
...
%%%
    ...
    \twocolumn
    \input{../Riferimenti/Appendice.tex}
    % \end{multicols}
