...
%%%
    ...
    \section{Complications, settings and results}\label{secResultsComparisons}

        In this section we first introduce complications to the base models seen in § \ref{secKineticModels}, then parameters and initial conditions are set, and finally the results are discussed comparing the base models with and without complications.
        ...
        \subsection{Results}\label{secResults}
            ...
            All simulations have been carried out with MATLAB software version «24.2.0.2923080 (R2024b) Update 6».
...
% %%%
    ...
    \section{Conclusions}\label{secConclusions}
        ...
        All results in § \ref{secResults} are coherent with the theory explained in § \ref{secKineticModels}, except for the $f_d^l\leftrightarrow f_d^m$ variant in the \FKdiscrete{} and \HDdiscrete{} models in which it's not clear why the medium value edge are so impactful on the dynamic, even though they are not the majority. A possible motivation comes from how the graph topology changes: it's reasonable to assume it's unaffected in the other cases, while in the $f_d^l\leftrightarrow f_d^m$ variant it varies significantly.
        
        As with most simulations, they have to be taken carefully; this is true especially from a biomedical point of view since the parameters used are purely theoretical and do not come from any study (§ \ref{secDisclaimer}).

        Furthermore, to confirm the results given with a dynamical Laplacian, its evolution in § \ref{secComplicationsDynamicLaplacian} has to be compared with actual measures in vivo, which is understandably difficult to achieve. It would also be interesting to find some laws about its evolution either based on ageing or concentrations of tau proteins, and hence their damage done to the brain.

        Another possible improvement can be searched in dynamic parameters, but once again the problem is to find their appropriate evolution; indeed, one could for instance expect in a diseased brain to have an increasing nucleation rate alongside a decreasing healthy clearance rate. Moreover, perhaps, their evolution is slow enough to justify their current constant value with an average.
